...
function renderOverview(){
 ...
 const markers=[['MOYENNE SKILL 1',average(p,0).toFixed(1)],['MOYENNE SKILL 2',average(p,1).toFixed(1)],['MOYENNE SKILL 3',average(p,2).toFixed(1)],['BOND 10+',bond10(p)],['NIVEAU 120',countLevel(p,120)],['% POSSÉDÉ 4★ / 5★',collectionPct(p).toFixed(1)+'%']];
...
async function fetchAtlasList(){try{const url='https://api.atlasacademy.io/nice/NA/servant/search?className=saber&className=archer&className=lancer&className=rider&className=caster&className=assassin&className=berserker&className=shielder&className=ruler&className=alterEgo&className=avenger&className=moonCancer&className=foreigner';const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw Error(r.status);const arr=await r.json();return Array.isArray(arr)?arr:[]}catch(e){console.warn('Atlas NA list indisponible',e);return[]}}
...
async function saveModal(id){const s=ensureStats(currentPlayer,id);s.level=Math.max(1,Math.min(120,num($('#e-level')?.value)||1));s.np=num($('#e-np')?.value);s.bond=Math.max(0,Math.min(15,num($('#e-bond')?.value)||0));s.grail=Math.max(0,Math.min(15,num($('#e-grail')?.value)||0));s.fouHp=Math.max(0,Math.min(1000,num($('#e-fouhp')?.value)||0));s.fouAtk=Math.max(0,Math.min(1000,num($('#e-fouatk')?.value)||0));s.skills=[0,1,2].map(i=>{const v=num($('#skill-'+i)?.value);return v==null?null:Math.max(1,Math.min(10,v))});s.appendSkills=[0,1,2,3,4].map(i=>{const v=num($('#append-'+i)?.value);return v==null?null:Math.max(0,Math.min(10,v))});localCacheSave();await persistStat(id);closeModal();renderAll();toast(cloudEnabled&&session&&currentCanEdit()?'Servant enregistré':'Servant enregistré localement')}
...
function initCompareNames(){const r=state.roster.filter(isVisible);$('#compareNames').innerHTML=r.map(x=>`<option value="${x.name}">${x.name}</option>`).join('');if(state.roster.some(x=>Number(x.id)===284))compareFocusId=284;}
...
async function saveXpCloud(){if(!cloudEnabled||!cloud||!session||!currentCanEdit())return;const inv=xpInventory(currentPlayer);const {error}=await cloud.from('chaldea_xp').upsert({player_key:currentPlayer,inventory:inv,updated_at:new Date().toISOString()},{onConflict:'player_key'});if(error)console.warn('XP sync',error.message)}
