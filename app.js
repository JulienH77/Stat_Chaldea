...
function openModalBase(r,d,urls,idx){
 ...
 if(can){
   ...
   btn.onmouseenter=()=>{const hover=Number(btn.dataset.bond);$$('[data-bond]').forEach(x=>x.classList.toggle('hovered',Number(x.dataset.bond)<=hover));};
   btn.onmouseleave=()=>$$('[data-bond]').forEach(x=>x.classList.remove('hovered'));
