...
function npIcon(){
  const f = NP_LEVEL_ICON;
  return `<img class="np-card-icon" src="${fandomAsset(f)}" alt="" loading="eager" ${fandomImgAttrs(f)}>`;
}
...
function openModalBase(r,d,urls,idx){
 ...
 const npHtml=can?`<div class="np-edit-wrap">${ni}<input class="editable-input np-input" id="e-np" type="number" value="${npValue}" min="1" max="999"><span class="np-display-hint">Affiché ${npRead}</span></div>`:`<div class="np-detail-row">${ni}<b>${npRead}</b></div>`;
...
function renderXp(){
  const p=currentPlayer,inv=xpInventory(p),can=currentCanEdit(),totals=xpTotalsFor(p);
  ...
  $$('.xp-stock-input').forEach(i=>i.oninput=()=>{inv[i.dataset.xpClass][i.dataset.xpStar]=Math.max(0,Math.floor(Number(i.value)||0));clearTimeout(i._cloudTimer);i._cloudTimer=setTimeout(saveXpCloud,600);updateXpComputed()});
