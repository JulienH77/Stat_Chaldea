...
function updateFilterLabels(){$('#classFilterCount').textContent=selectedClasses.size?`(${selectedClasses.size})`:'';$('#rarityFilterCount').textContent=selectedRarities.size?`(${selectedRarities.size})`:'';const b=$('#hideMissing');if(b){b.classList.toggle('active',hideMissing);b.textContent=hideMissing?'Afficher les non possédés':'Masquer les non possédés'}}
