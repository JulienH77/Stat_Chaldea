...
function bondDiamonds(bond,editable=false){
 ...
 const row=(start,count,rowNo)=>Array.from({length:count},(_,j)=>{const i=start+j+1;const tag=editable?'button':'span';const type=editable?' type="button"':'';return `<${tag}${type} class="bond-diamond ${i<=n?'filled':''}" ${editable?`data-bond="${i}"`:''} aria-label="Bond ${i}"></${tag}>`}).join('');
 return `<div class="bond-diamonds ${editable?'bond-editable':''}" id="bondDiamonds"><div class="bond-row bond-row-1">${row(0,10,1)}</div><div class="bond-row bond-row-2">${row(10,5,2)}</div></div><small class="bond-caption" id="bondCaption">Bond ${n||'—'} / 15${editable?' · cliquez sur un losange':''}</small>`;
}
...
function openModalBase(r,d,urls,idx){
 ...
 if(can){
   ...
   $$('[data-bond]').forEach(btn=>btn.onclick=()=>{const value=Number(btn.dataset.bond);bondInput.value=value;$$('[data-bond]').forEach(x=>x.classList.toggle('filled',Number(x.dataset.bond)<=value));if(bondCaption)bondCaption.textContent=`Bond ${value} / 15 · cliquez sur un losange`;});
