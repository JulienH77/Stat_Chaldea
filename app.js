...
let currentPlayer='julien',currentView='overview',rosterMode='cards',sortDir='desc',compareFocusId=284,skillScope='all';
...
function averageLevel(p){const a=state.roster.filter(r=>isCounted(r)&&isOwned(p,r)).map(r=>displayLevel(p,r,stats(p,r.id))).filter(Number.isFinite);return a.length?a.reduce((x,y)=>x+y,0)/a.length:0}
function compareStats(p){return{owned:countOwned(p),five:countOwned(p,r=>rarityNum(r.rarity)===5&&!isWelfare(r)),four:countOwned(p,r=>rarityNum(r.rarity)===4&&!isWelfare(r)),wf:countOwned(p,isWelfare),a1:average(p,0),a2:average(p,1),a3:average(p,2),lvl100:countLevel(p,100),lvl120:countLevel(p,120),bond:bond10(p),avgLevel:averageLevel(p)}}
function renderCompare(){const vals=PLAYERS.map(p=>({p,...compareStats(p)})),tones=['#c6535f','#4f78b7','#579b76'];$('#compareTop').innerHTML=vals.map(v=>`<article class="person-card"><div class="person-name">${PLAYER_LABELS[v.p]}</div><div class="person-score">${v.owned}</div><small class="person-label">SERVANTS POSSÉDÉS</small><div class="collection-note"><span>COLLECTION</span><b>${v.owned}</b><small>${collectionPct(v.p).toFixed(1)}% des 4★/5★</small></div><div class="person-meta"><span><small>5★</small><b>${v.five}</b></span><span><small>4★</small><b>${v.four}</b></span><span><small>WELFARE</small><b>${v.wf}</b></span><span><small>SKILL 1 MOY.</small><b>${v.a1.toFixed(1)}</b></span><span><small>SKILL 2 MOY.</small><b>${v.a2.toFixed(1)}</b></span><span><small>SKILL 3 MOY.</small><b>${v.a3.toFixed(1)}</b></span><span><small>BOND 10+</small><b>${v.bond}</b></span><span><small>LV 120</small><b>${v.lvl120}</b></span><span><small>NIVEAU MOY.</small><b>${v.avgLevel.toFixed(1)}</b></span></div></article>`).join('');initCompareNames();renderShowdown();renderRadar(tones)}
