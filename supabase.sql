-- Chaldea Command V4 — Supabase schema
-- Create exactly the three Master users in Auth first, then associate them here.
...
create policy stats_read_authenticated on public.chaldea_stats for select to authenticated using(true);
drop policy if exists stats_insert_own_player on public.chaldea_stats;
create policy stats_insert_own_player on public.chaldea_stats for insert to authenticated with check(
 player_key=(select m.player_key from public.chaldea_members m where m.auth_user_id=auth.uid() and m.can_edit=true)
);
drop policy if exists stats_update_own_player on public.chaldea_stats;
create policy stats_update_own_player on public.chaldea_stats for update to authenticated using(
 player_key=(select m.player_key from public.chaldea_members m where m.auth_user_id=auth.uid() and m.can_edit=true)
) with check(
 player_key=(select m.player_key from public.chaldea_members m where m.auth_user_id=auth.uid() and m.can_edit=true)
);
drop policy if exists stats_delete_own_player on public.chaldea_stats;
create policy stats_delete_own_player on public.chaldea_stats for delete to authenticated using(
 player_key=(select m.player_key from public.chaldea_members m where m.auth_user_id=auth.uid() and m.can_edit=true)
);
